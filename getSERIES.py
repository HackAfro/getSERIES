...
def check(url):
    ...
    try:
        ...
        print(url)
        season_page = make_request(url)  # gets the page your tv-serie
        ...
        get_img = make_request(img_src)

        for link in links:
            seasons = make_request(link.get('href'))
        ...
        episode_page = make_request(links[0].get('href'))
...
def checked():
    ...
    if on_off == 1:
        ...
        season_entry['values'] = ['Season-0{}'.format(i) if len(str(i)) == 1 else 'season-{}'.format(i) for i in
                                  range(1, 11)][0:]
        season_entry.current(0)

        episode_entry['values'] = ['episode-0{}'.format(i) if len(str(i)) == 1 else 'episode-{}'.format(i) for i in
                                   range(1, 25)][0:]
        episode_entry.current(0)
...
def seaClicked(*args):
    ...
    if seaS() in the_episodes.keys():
        ...
        try:
            episode_entry['values'] = ['E0{}'.format(i) if len(str(i)) == 1 else 'E{}'.format(i) for i in
                                       range(1, max_epi)][0:]
            episode_entry.current(0)
        except TypeError:
            pass
...
def grab_episodes():
    try:
        with open('url.txt', 'r') as page, open('episode_new.txt', 'w') as file, open('series_new.txt',
                                                                                      'w') as epis, open(
                'today_url.txt', 'r') as page1, open('today_new.txt', 'w') as today, open('today_epi_new.txt',
                                                                                          'w') as today_epi:
            ...
            for j in range(0, 240, 12):
                ...
                tv_series = make_request(hd + shift)
